...
public:
  ...
  virtual double getJacobian(const CellVertices<2>& vertices, const MeshEntity& localEntity, const vertex<2>& v) const = 0;
  virtual vertex<2> reference_to_physical(const CellVertices<2>& vertices, const vertex<2>& vertex) const = 0;
  virtual Tensor<2, 1, double> getFacetNormal(const CellVertices<2>& vertices, const std::size_t fid,
    const vertex<2>& v) const = 0;
  virtual ~GeneralCell() {}
};
