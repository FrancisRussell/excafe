...
private:
  ...
  void visitTuple(NumericExpressionVisitor<V>& v, const boost::python::tuple& tuple) const
  {
    ...
    else if (opType == FLOAT)
    {
      ...
      v.visitConstant(excafe::numeric_cast<typename NumericExpressionVisitor<V>::float_t>(constant));
    }
    else if (opType == INTEGER)
    {
      const long constant = boost::python::extract<long>(operand);
      v.visitConstant(excafe::numeric_cast<typename NumericExpressionVisitor<V>::integer_t>(constant));
    }
    else if (opType == SYM)
    {
